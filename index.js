...
const atende = new Date().getUTCHours() < 8 ? 'Humm... \n😒 Já estamos fora do horário de atendimento.\n\n😃 Mas não se preocupe, retornaremos assim que possível!\n\n🕖 _Nosso horário é de segunda a sábado de 08:00hs às 20:00hs._' : new Date().getUTCHours() < 20 ? '😃 Aguarde um momento que logo será atendido.' : 'Humm... \n😒 Já estamos fora do horário de atendimento.\n\n😃 Mas não se preocupe, retornaremos assim que possível!\n\n🕖 _Nosso horário é de segunda a sábado de 08:00hs às 20:00hs._';
const saudacao = new Date().getUTCHours() < 6 ? '*Boa Madrugada!*' : new Date().getUTCHours() < 12 ? '*Bom diaa!*' : new Date().getUTCHours() < 18 ? '*Boa tarde!*' : '*Boa noite!*';
const domingo = new Date().getUTCDay() === 0 ? '🏖️ *Hoje é Domingo!*\n\n😃 Amanhã logo cedo um de nossos atendentes irá falar com você.\n\n🕖 _Nosso horário é de segunda a sábado de 08:00hs às 20:00hs._' : new Date().getUTCDay() > 0 ? atende: '🏖️ *Hoje é Domingo!*\n\n😃 Amanhã logo cedo um de nossos atendentes irá falar com você.\n\n🕖 _Nosso horário é de segunda a sábado de 08:00hs às 20:00hs._';
...
client.on('message', async msg => {
    ...
    } else if (msg.body === ("Cozinha" || "Sala" || "Banheiro" || "Quarto")){
        msg.react('👍');
    ...
    else if (msg.body === ("Versatile" || "Vigneto" || "Sicilia" || "Venezia" || "Toscana" || "Vita" || "Nuvole" || "Firenze" || "Eleganza" || "Sofisticato")) {
