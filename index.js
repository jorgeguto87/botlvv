...
client.on('message', async msg => {
    ...
    else if (horaautal < inicioatend || horaautal > fimatend) {
        const contact = msg.from;
        const texto = `Mensagem recebida pela La Vita de ${contact}: ${msg.body}`;
        await client.sendMessage (jorge, texto);
        await client.sendMessage(marcus, texto);

    }else if (diaautal === folga) {
        const contact = msg.from;
        const texto = `Mensagem recebida pela La Vita de ${contact}: ${msg.body}`;
        await client.sendMessage (jorge, texto);
        await client.sendMessage(marcus, texto);
