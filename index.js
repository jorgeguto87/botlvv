...
async function scheduleAdvertisements() {
    ...
    try {
        ...
        if (rows.length) {
            for (const [index, row] of rows.entries()) {
                const [name, phone, status, date, hour1, hour2, hour3, dateok, hourok, apres] = row;
                ...
                if (status === 'pronto') {
                    ...
                    // Atualizando status para "Notificado"
...
async function main() {
    try {
        // 1. Inicia fluxo principal de atendimento
        handleClientMessages();

        // 2. Inicia anúncios programados
        scheduleAdvertisements();

        // 3. Executa integração com a planilha de forma assíncrona
        setInterval(() => {
            addDataToSheet();
